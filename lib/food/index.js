...
async function getOfficer(dayOfWeek) {
  ...
  return new Promise((resolve, reject) => {
    request(cafeteriaUtils.getSoonguriJsonUrl(dayOfWeek), (err, response, body) => {
      if (!err) {
        const ret = {
          kind: 'officer',
          dayOfWeek,
    ...
    request({
      ...
      if (!err) {
        ...
        const ret = {
          ...
          date: '{year}-{month}-{day}'.formatUnicorn({year, month, day}),
...
async function getPrice(menuList) {
  ...
  return new Promise((resolve, reject) => {
    request(cafeteriaUtils.getSsuDesktopUrl(), (err, response) => {
      if (!err) {
